...
FILE_PATH = os.path.join('templates', 'projects', 'detail_fixed.html')
...
<div class="container">
    <div class="page-header">
        ...
        <div class="flex gap-2">
            ...
            <button type="button" class="btn btn-error" onclick="confirmDeleteProject()">Excluir Projeto</button>
        </div>
    </div>
    ...
    <div class="card mb-6">
        ...
        <div class="card-body">
            ...
            <form method="POST" action="{% url 'projects:rss_settings' project.id %}"
                ...
                <div class="grid-2 gap-4">
                    <div class="form-group">
                        ...
                        <select name="check_interval_minutes" class="form-input">
                            <option value="30" {% if is_interval_30 %}selected{% endif %}>30 minutos</option>
                            <option value="60" {% if is_interval_60 %}selected{% endif %}>1 hora</option>
                            <option value="120" {% if is_interval_120 %}selected{% endif %}>2 horas</option>
                            <option value="360" {% if is_interval_360 %}selected{% endif %}>6 horas</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Máximo de Posts/Dia (Total)</label>
                        <select name="max_posts_per_day" class="form-input">
                            <option value="3" {% if is_max_posts_3 %}selected{% endif %}>3 posts</option>
                            <option value="5" {% if is_max_posts_5 %}selected{% endif %}>5 posts</option>
                            <option value="10" {% if is_max_posts_10 %}selected{% endif %}>10 posts</option>
                            <option value="20" {% if is_max_posts_20 %}selected{% endif %}>20 posts</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="grid-2 gap-4">
                    <div class="form-group">
                        <label class="checkbox-label">
                            <input type="checkbox" name="is_active" class="checkbox" {% if rss_is_active %}checked{% endif %}>
                            <span>Ativar monitoramento global</span>
                        </label>
                    </div>
                    <div class="form-group">
                        <label class="checkbox-label">
                            <input type="checkbox" name="auto_publish" class="checkbox" {% if rss_auto_publish %}checked{% endif %}>
                ...
                <div class="form-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="download_images" class="checkbox" {% if rss_download_images %}checked{% endif %}>
                ...
                <div class="form-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="include_source_attribution" class="checkbox" {% if rss_source_attribution %}checked{% endif %}>
                        ...
                        <tr>
                            ...
                            <td>
                                <form method="POST" action="{% url 'projects:rss_feed_delete' project.id feed.id %}"
                                    onsubmit="return confirmDeleteFeed(event);"
                                    style="display:inline;">
                                    {% csrf_token %}
                                    <button type="submit" class="text-error font-bold px-2" title="Remover">🗑️</button>
                                </form>
                            </td>
                        </tr>
                        {% endfor %}
            ...
            <div class="text-center p-6 border border-dashed rounded bg-gray-50">
                ...
                <div class="mt-2">
                    <button type="button" class="btn btn-primary" onclick="openRSSFeedModal()">Adicionar o primeiro
                        feed</button>
                </div>
            </div>
            {% endif %}
            ...
            <form method="POST" action="{% url 'projects:rss_feed_create' project.id %}">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="url" name="feed_url" class="form-input" required
    ...
    <script>
        ...
        // RSS Feed Deletion
        async function confirmDeleteFeed(event) {
            event.preventDefault();
            const form = event.target;
            
            const confirmed = await window.confirmAction({
                title: 'Remover Feed RSS?',
                message: 'Tem certeza que deseja remover este feed? O histórico será mantido.',
                type: 'danger',
                confirmText: 'Remover',
...
<script>
    ...
    var DELETE_ITEM_URL_BASE = `/projects/${PROJECT_ID}/editorial-item/`;
    var BULK_DELETE_URL = `/projects/${PROJECT_ID}/editorial-items/bulk-delete/`;
    ...
    // Modern Confirm Action Wrapper for Item deletion (checkbox support)
    async function confirmDeleteItem(itemId, title) {
        const content = document.createElement('div');
        content.innerHTML = `
            <p class="mb-4">Tem certeza que deseja excluir "<strong>${title}</strong>"?</p>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" id="delete-from-wp-single" class="checkbox">
                <span class="text-sm">Também excluir do WordPress (se publicado)</span>
            </label>
        `;

        const confirmed = await window.confirmAction({
            title: 'Excluir Item do Plano?',
            contentElement: content, // Custom content support needed or simplified
            html: true, // Assuming confirmAction supports HTML or we use message
            message: '', // Using content element trick if supported, OR revert to standard message
            type: 'warning',
            confirmText: 'Excluir',
            confirmClass: 'btn-error'
        });
        
        // Note: The global confirm-modal.js provided earlier is simple and takes 'message'.
        // For the checkbox functionality ('delete from wordpress'), we need a slightly more advanced modal 
        // OR we can simplify it for now to just prompt text, as the native confirm didn't have a checkbox easily either 
        // actually existing code HAD a checkbox in the custom modal '#delete-modal'.
        // We should probably keep the custom #delete-modal IF it has specific business logic like the checkbox,
        // BUT rewrite it to use the new standard if possible.
        // HOWEVER, the task is to REPLACE native alerts/confirms.
        // The existing file ALREADY had a custom 'delete-modal' (lines 474-494).
        // It wasn't using native confirm for deleteProject (line 690 calls openDeleteModal).
        // BUT it WAS using native confirm for feed delete (line 192) and delete all (line 662).
        
        // Let's stick to the brief: Replace NATIVE confirm/alert.
        // The existing 'delete-modal' is actually fine visually but we can uniform it.
        // To be safe and clean, I will implement a custom Confirm flow for the checkbox using SweetAlert style
        // OR reuse the existing logic but cleaned up.
        // Let's use window.confirmAction but since it doesn't support custom inputs (checkbox), 
        // I will simplify the requirement OR ask the user? No, I must act.
        // I will keep the checkboxes by passing a specialized message or verifying if confirmAction supports HTML.
        // Checking confirm-modal.js... it sets `modalMessage.textContent = config.message`. It does NOT support HTML.
        
        // DECISION: For 'Delete Item', I will modify the flow to ask "Delete from WordPress too?" in a separate step 
        // OR just keep it simple.
        // actually, looking at the previous code, the 'delete-modal' was ALREADY a custom modal with a checkbox.
        // I should probably KEEP that custom modal for Item Deletion since it has specific functional fields (checkbox),
        // but replace the NATIVE confirms for 'Delete All' and 'Delete Feed'.
        // Wait, the user wants to migrate to the DESIGN SYSTEM.
        // If the Design System's confirmAction doesn't support checkboxes, I should probably extend it or use the custom modal code 
        // but verify it looks correct.
        
        // Let's use the standard confirmAction for standard confirms.
        // For the complex 'Delete Item with Checkbox', I will RE-IMPLEMENT it using the new DOM elements from confirm-modal.js 
        // if I can, OR just keep the custom HTML but make sure it uses standard classes.
        // The existing 'delete-modal' seems to use .modal-backdrop and .modal which ARE from the design system.
        // So I will KEEP the custom delete-modal for Items/Project because it needs the Checkbox,
        // BUT I will replace the NATIVE confirms in 'Delete All' and 'Delete Feed'.
    }

    // Re-implementing Item Delete using the existing custom modal for the Checkbox feature
    // because confirmAction doesn't support inputs yet.
    // However, I will clean up the code.
    
    // ... Actually, to ensure full migration, I'll extend confirmAction slightly locally or just manually handle the modal 
    // for the complex cases, and use confirmAction for the simple ones.
    
    // Let's look at confirmDeleteAllItems (Line 661). It uses native confirm.
    // Let's look at feed delete (Line 192). It uses native confirm.
    
    async function confirmDeleteAllItems() {
        const confirmed = await window.confirmAction({
            title: '⚠️ Excluir Todos os Itens?',
            message: 'Isso excluirá TODOS os itens do plano editorial atual. Esta ação não pode ser desfeita.',
            type: 'danger',
        ...
        if (confirmed) {
            ...
            // Trigger deletion via fetch or redirect
             try {
                ...
                if (response.ok) {
                    window.showToast('Todos os itens excluídos', 'success');
                    setTimeout(() => location.reload(), 1500);
                } else {
                    window.showToast('Erro ao excluir itens', 'error');
                }
            } catch (e) {
    ...
    // PROJECT DELETION
    var PROJECT_DELETE_URL = `/projects/${PROJECT_ID}/delete/`;

    async function confirmDeleteProject() {
        const confirmed = await window.confirmAction({
            title: '⚠️ Excluir Projeto?',
            message: 'Isso apagará PERMANENTEMENTE todos os posts, histórico e configurações. Esta ação NÃO pode ser desfeita.',
            type: 'danger',
        ...
        if (confirmed) {
            ...
            form.method = 'POST';
            form.action = PROJECT_DELETE_URL;
    ...
    // ITEM DELETION (Complex with Checkbox)
    // I will keep the custom modal Logic from the original file but rewritten to be cleaner
    // and ensuring it matches the styles.
    
    // Since I'm rewriting the file, I need to include the HTML for the complex modal?
    // Or can I be clever?
    // The previous file had <div id="delete-modal">...
    // I will INCLUDE IT again, but verify classes interact with design-system.css
</script>

<!-- Custom Modal for Delete Item with Checkbox Option -->
<div id="delete-item-modal" class="modal-backdrop">
    <div class="modal">
        <div class="modal-header">
            <h3 class="modal-title">Confirmar Exclusão</h3>
            <button class="modal-close" onclick="closeDeleteItemModal()">&times;</button>
        </div>
        <div class="modal-body">
            <p id="delete-item-msg">Tem certeza?</p>
        ...
        <div class="modal-footer">
            <button class="btn btn-ghost" onclick="closeDeleteItemModal()">Cancelar</button>
            <button class="btn btn-error" id="btn-confirm-delete-item">Excluir</button>
        </div>
    </div>
</div>

<script>
    // Logic for the custom item delete modal
    let itemDeleteCallback = null;
    const deleteItemModal = document.getElementById('delete-item-modal');

    function openDeleteItemModal(msg, callback) {
        document.getElementById('delete-item-msg').innerHTML = msg;
    ...
    document.getElementById('btn-confirm-delete-item').addEventListener('click', () => {
        if (itemDeleteCallback) {
            const fromWp = document.getElementById('delete-from-wp').checked;
            itemDeleteCallback(fromWp);
        }
        closeDeleteItemModal();
    });

    // Helper to wire it up
    window.confirmDeleteItem = function(itemId, title) {
        openDeleteItemModal(
            `Excluir o item "<strong>${title}</strong>"?`,
    ...
    function bulkDeleteItems() {
        ...
        openDeleteItemModal(
             `Excluir ${selectedItems.size} itens selecionados?`,
             function(deleteFromWp) {
    ...
    async function performBulkDelete(deleteFromWp) {
        try {
            ...
            if (response.ok) {
                window.showToast('Itens excluídos', 'success');
