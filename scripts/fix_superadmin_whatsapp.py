...
<script>
    // JAVASCRIPT COM LOGGING DETALHADO E DESIGN SYSTEM
    async function startConnection() {
        ...
        try {
            ...
            if (!createData.success) {
                window.showToast('Erro ao criar usuário: ' + createData.message, 'error');
                btn.disabled = false;
            ...
            if (!connectData.success) {
                window.showToast('Erro ao conectar: ' + connectData.message + ' - Detalhe: ' + (connectData.detail || 'N/A'), 'error');
                btn.disabled = false;
                btn.innerText = 'Iniciar Conexão';
                return;
            }
    ...
    async function resetConfig() {
        const confirmed = await window.confirmAction({
            ...
            message: 'Isso irá desconectar o WhatsApp e limpar todos os dados locais. Esta ação não pode ser desfeita.',
            type: 'danger',
            confirmText: 'Resetar',
            cancelText: 'Cancelar'
        });
        ...
        try {
            ...
            if (data.success) {
                window.showToast('Configuração resetada com sucesso!', 'success');
                setTimeout(() => location.reload(), 1500);
            } else {
                window.showToast('Erro ao resetar: ' + (data.message || 'Erro desconhecido'), 'error');
            }
        } catch (error) {
            window.showToast('Erro ao resetar: ' + error.message, 'error');
        }
    }
    ...
    async function disconnectWhatsApp() {
        const confirmed = await window.confirmAction({
            ...
            message: 'Deseja realmente desconectar o WhatsApp? Você precisará escanear o QR Code novamente para reconectar.',
            type: 'warning',
        ...
        try {
            ...
            if (data.success) {
                ...
                window.showToast('Erro ao desconectar: ' + (data.message || 'Erro desconhecido'), 'error');
            }
        } catch (error) {
            window.showToast('Erro ao desconectar: ' + error.message, 'error');
        }
    }
</script>
{% endblock %}
