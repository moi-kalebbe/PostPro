            ...
            <form method="get" class="flex gap-4 items-end flex-wrap">
                ...
                <div class="form-group mb-0 flex-1" style="min-width: 200px;">
                    ...
                    <input type="text" name="search" class="form-input" value="{{ search }}"
                        placeholder="Keyword ou título..." data-search>
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header bulk-actions-bar" id="bulk-actions-bar" style="display: none;">
            <div class="flex items-center gap-4">
                <span class="text-sm"><span id="selected-count">0</span> post(s) selecionado(s)</span>
                <button type="button" class="btn btn-error btn-sm" onclick="bulkDeletePosts()">
                    ...
                    <tr data-post-id="{{ post.id }}">
                        ...
                        <td>
                            <div class="flex gap-1">
                                <button class="btn btn-ghost btn-sm" onclick="openPostModal('{{ post.id }}')"
                                    title="Preview">
                                ...
                                <button class="btn btn-ghost btn-sm" onclick="publishPost('{{ post.id }}')"
                                    title="Publicar">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <a href="{{ post.wordpress_edit_url }}" target="_blank" class="btn btn-ghost btn-sm"
                                    title="Editar no WP">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <button class="btn btn-ghost btn-sm text-error"
                                    onclick="openDeleteModal('{{ post.id }}', '{{ post.keyword|truncatewords:3 }}', {{ post.wordpress_post_id|default:'null' }})"
                                    title="Deletar">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
                                        fill="none" stroke="currentColor" stroke-width="2">
                                        <polyline points="3 6 5 6 21 6"></polyline>
        ...
        <div class="card-footer">
            <div class="flex justify-center gap-2">
                ...
                <a href="?page={{ posts.previous_page_number }}&project={{ project_filter }}&status={{ status_filter }}&search={{ search }}"
                ...
                {% if posts.has_next %}
                <a href="?page={{ posts.next_page_number }}&project={{ project_filter }}&status={{ status_filter }}&search={{ search }}"
                    class="btn btn-secondary btn-sm">Próxima</a>
                {% endif %}
            </div>
        </div>
        {% endif %}
...
<script>
    ...
    let deleteHasWordPress = false;

    function openDeleteModal(postId, keyword, wpPostId) {
        deletePostId = postId;
        deleteHasWordPress = wpPostId !== null && wpPostId !== 'null';

        document.getElementById('delete-post-keyword').textContent = keyword;

        const wpOption = document.getElementById('delete-wp-option');
        if (deleteHasWordPress) {
            wpOption.style.display = 'block';
        } else {
            wpOption.style.display = 'none';
    ...
    async function confirmDeletePost() {
        ...
        try {
            const response = await fetch('/automation/posts/' + deletePostId + '/delete/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('postpro_csrftoken'),
            ...
            if (data.success) {
                ...
                if (data.deleted_from_wordpress) {
                    showToast('Também removido do WordPress', 'info');
                }
            } else {
    ...
    async function bulkDeletePosts() {
        ...
        if (postIds.length === 0) {
            showToast('Nenhum post selecionado', 'warning');
            return;
        }

        // Open bulk delete modal
        document.getElementById('bulk-delete-count').textContent = postIds.length;
        openModal('bulk-delete-modal');
    }

    async function confirmBulkDelete() {
        const postIds = getSelectedPostIds();
        const deleteFromWp = document.getElementById('bulk-delete-from-wp').checked;

        try {
            const response = await fetch('/automation/posts/bulk-delete/', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRFToken': getCookie('postpro_csrftoken'),
                ...
                body: JSON.stringify({
                    post_ids: postIds,
                    delete_from_wordpress: deleteFromWp
                }),
            });
            const data = await response.json();
            ...
            if (data.success) {
                ...
                if (data.wp_deleted_count > 0) {
                    showToast(`${data.wp_deleted_count} removido(s) do WordPress`, 'info');
                }
...
<div class="modal-backdrop" id="delete-modal">
    <div class="modal" style="max-width: 450px;">
        ...
        <div class="modal-body">
            <p>Tem certeza que deseja deletar o post "<strong id="delete-post-keyword"></strong>"?</p>
            <div id="delete-wp-option" class="mt-4" style="display: none;">
                <label class="flex items-center gap-2">
                    <input type="checkbox" id="delete-from-wp">
                    <span>Deletar também do WordPress</span>
                </label>
                <p class="text-sm text-muted mt-1">O post será movido para a lixeira no WordPress.</p>
            </div>
        </div>
        <div class="modal-footer">
            <button class="btn btn-ghost" onclick="closeModal('delete-modal')">Cancelar</button>
            <button class="btn btn-error" onclick="confirmDeletePost()">Deletar</button>
        </div>
    </div>
</div>
...
<div class="modal-backdrop" id="bulk-delete-modal">
    <div class="modal" style="max-width: 450px;">
        ...
        <div class="modal-body">
            <p>Tem certeza que deseja deletar <strong id="bulk-delete-count">0</strong> post(s)?</p>
            <div class="mt-4">
                <label class="flex items-center gap-2">
                    <input type="checkbox" id="bulk-delete-from-wp">
                    <span>Deletar também do WordPress</span>
                </label>
                <p class="text-sm text-muted mt-1">Os posts publicados serão movidos para a lixeira.</p>
            </div>
        </div>
        <div class="modal-footer">
