...
file_path = r'c:\Users\olx\OneDrive\Desktop\PROJETOS 2026\PostPro\templates\projects\detail_fixed.html'

# The correct content with STRICT single-line tags
...
<script>
    ...
    // API URLs - var to avoid redeclaration errors
    ...
    window.openDeleteModal = function(message, callback, showCheckbox = true) {
        const modal = document.getElementById('delete-modal');
        document.getElementById('delete-modal-message').innerHTML = message.replace(/\\n/g, '<br>');
        
        const checkboxGroup = document.getElementById('delete-from-wp').closest('.form-group');
        if (showCheckbox) {
    ...
    function confirmDeleteAllItems() {
        ...
        // Implement delete all logic if needed or redirect
    }
    ...
    window.confirmDeleteProject = function() {
        const warningMsg = '⚠️ <strong>PERIGO:</strong> Tem certeza que deseja excluir este projeto?\\n\\nIsso apagará PERMANENTEMENTE:\\n- Todos os posts gerados\\n- Histórico e estatísticas\\n- Configurações e feeds\\n\\nEsta ação NÃO pode ser desfeita.';
        
        openDeleteModal(warningMsg, function() {
...
try:
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("Successfully overwrote file with single-line tags.")
